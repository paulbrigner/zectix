...
import {
  ...
  readJsonOrThrow,
  testWebhookCallbackUrl,
} from "@/app/test/client-utils";
...
export function TestAdminClient() {
  ...
  async function loadConfig() {
    ...
    try {
      const response = await readJsonOrThrow<ConfigResponse>(
        await fetch("/api/test/config", { cache: "no-store" }),
      );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setWebhookSecret("");
      setLumaApiKey("");
      setNotice("Test config saved.");
    } catch (saveError) {
      setError(
        saveError instanceof Error
          ? saveError.message
          : "Failed to save test config",
      );
    } finally {
      setSaving(false);
    }
  ...
  const lumaWebhookUrl = origin ? lumaWebhookCallbackUrl(origin) : "/api/luma/webhook";
        ...
        <header className="test-section-header">
          <div>
            ...
            <p className="subtle-text">
              Store the local test harness settings for CipherPay and Luma.
            </p>
          </div>
        </header>
