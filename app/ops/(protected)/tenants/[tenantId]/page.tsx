...
function summarizeCalendarInventory(
  ...
  const mirroredEvents =
    detail?.events.find(
      (entry) =>
        entry.calendar.calendar_connection_id ===
    ...
    <>
      <section className="console-section">
        <div className="console-section-header">
          <div>
            ...
            <p className="subtle-text">
              {detail.tenant.contact_email} · {detail.tenant.service_fee_bps}{" "}
              bps service fee · billing {detail.tenant.billing_status}
            </p>
          </div>
          <div className="button-row">
            <Link
              className="button button-secondary button-small"
              href={`/ops/tenants/${encodeURIComponent(detail.tenant.tenant_id)}/dashboard`}
            >
              Dashboard
            </Link>
            <Link
        ...
        <form action={setTenantStatusAction} className="console-content">
          ...
          <p className="subtle-text">
            Public calendar pages only resolve for organizations whose status is{" "}
            <code>active</code>.
          </p>
          <button
            className="button button-secondary button-small"
      ...
      <section className="console-section">
        <div className="console-section-header">
          <div>
            ...
            <p className="subtle-text">
              Platform fees are tracked natively in ZEC. These controls manage
              delinquency, grace handling, and when a cycle is large enough to
              settle.
            </p>
          </div>
          {detail.billing?.current_cycle ? (
            <article className="console-detail-card">
              ...
              <p className="subtle-text">
                Outstanding{" "}
                {formatZecAmount(
                  detail.billing.current_cycle.outstanding_zatoshis,
                )}
              </p>
            </article>
          ) : null}
              ...
              >
                <div className="console-luma-card-head">
                  <div>
                    ...
                    <p className="subtle-text">
                      Public URL:{" "}
                      <Link href={publicCalendarHref}>/c/{calendar.slug}</Link>
                    </p>
                  </div>
                  <div className="button-row">
                    ...
                    {calendar.status !== "disabled" ? (
                      <ConsoleConfirmDialog
                        ...
                        description="This will turn off public checkout for this calendar until it is connected and validated again."
                ...
                <div className="console-signal-grid">
                  ...
                  <div className="console-signal-card">
                    ...
                    <strong>
                      {futureMirroredEvents.length} future events ·{" "}
                      {ticketCount} tickets
                    </strong>
                    <p className="subtle-text">
                      {enabledEvents.length} events and {enabledTickets.length}{" "}
                      tickets are currently Zcash-enabled · {liveOnlyCount}{" "}
                      still only visible in Luma
                    </p>
                  </div>
                  <div className="console-signal-card">
                    <span className="console-kpi-label">Last sync</span>
                    <strong>
                      {calendar.last_synced_at ? "Completed" : "Not yet synced"}
                    </strong>
                    <p className="subtle-text">
                ...
                {calendar.status === "disabled" ? (
                  <p className="subtle-text">
                    This calendar is disabled. Public checkout is off for this
                    calendar until you run <code>Re-enable and sync</code>.
                  </p>
                ) : null}
                ...
                <div className="console-inline-action">
                  <p className="subtle-text">
                    ...
                    {liveEventCount === 1 ? "" : "s"} available from the saved
                    key.
                  </p>
                  <ConsoleInfoTip label="What Validate and sync does">
                    <p>
                      ...
                      <code>event.created</code>, <code>event.updated</code>,
                      and <code>event.canceled</code>, then refreshes the
                      mirrored events and tickets used by public checkout.
                    </p>
                  </ConsoleInfoTip>
                </div>
                ...
                {livePreview?.error ? (
                  ...
                  <div className="console-preview-list">
                    {nextLiveEvents.map((event) => {
                      const mirroredEvent =
                        mirroredByEventId.get(event.api_id) || null;
                  ...
                  <div className="console-preview-empty">
                    ...
                    <p className="subtle-text">
                      Once this key can read upcoming Luma events, they will
                      appear here before and after each sync.
                    </p>
                  </div>
                )}
              </article>
        ...
        <div className="console-section-header">
          <div>
            ...
            <p className="subtle-text">
              ...
              this form for a calendar replaces that calendar’s current live
              checkout connection instead of adding another active one.
            </p>
          </div>
        </div>
